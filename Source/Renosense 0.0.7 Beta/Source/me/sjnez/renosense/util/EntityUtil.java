...
public class EntityUtil implements Util
{
    ...
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals((Object)EntityUtil.mc.player) || (entity instanceof EntityPlayer && RenoSense.friendManager.isFriend(entity.getName())) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            if (player.getName().equalsIgnoreCase(EntityUtil.mc.player.getName()) && !(boolean)HUD.getInstance().selfTextRadar.getValue()) {
                continue;
            }
            ...
            output.put(healthSB.toString() + " " + (RenoSense.friendManager.isFriend(player) ? "§a" : (RenoSense.enemyManager.isSuperEnemy(player) ? "§4§l" : (RenoSense.enemyManager.isEnemy(player) ? "§c" : "§r"))) + player.getName() + " " + distanceSB.toString() + " §f" + RenoSense.totemPopManager.getTotemPopString(player) + RenoSense.potionManager.getTextRadarPotion(player), (int)EntityUtil.mc.player.getDistance((Entity)player));
    ...
    public static double toRadian(final double d) {
        return d * 3.141592653589793 / 100.0;
    }
