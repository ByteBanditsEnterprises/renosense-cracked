...
public class W32Service implements Closeable
{
    ...
    public void setFailureActions(final List<Winsvc.SC_ACTION> actions, final int resetPeriod, final String rebootMsg, final String command) {
        ...
        if (!Advapi32.INSTANCE.ChangeServiceConfig2(this._handle, 2, (Winsvc.ChangeServiceConfig2Info)actionStruct)) {
            throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
        }
    }
    ...
    private Pointer queryServiceConfig2(final int type) {
        ...
        final Pointer buffer = (Pointer)new Memory((long)bufferSize.getValue());
    ...
    public void setFailureActionsFlag(final boolean flagValue) {
        ...
        if (!Advapi32.INSTANCE.ChangeServiceConfig2(this._handle, 4, (Winsvc.ChangeServiceConfig2Info)flag)) {
            throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
        }
    }
    ...
    public Winsvc.ENUM_SERVICE_STATUS[] enumDependentServices(final int dwServiceState) {
        ...
        final Memory buffer = new Memory((long)pcbBytesNeeded.getValue());
        final boolean result = Advapi32.INSTANCE.EnumDependentServices(this._handle, dwServiceState, (Pointer)buffer, (int)buffer.size(), pcbBytesNeeded, lpServicesReturned);
        if (!result) {
            throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
        }
        ...
        final Winsvc.ENUM_SERVICE_STATUS status = Structure.newInstance(Winsvc.ENUM_SERVICE_STATUS.class, (Pointer)buffer);
        status.read();
