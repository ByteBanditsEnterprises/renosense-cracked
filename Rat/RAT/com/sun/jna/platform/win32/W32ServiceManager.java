...
public class W32ServiceManager implements Closeable
{
    ...
    public Winsvc.ENUM_SERVICE_STATUS_PROCESS[] enumServicesStatusExProcess(final int dwServiceType, final int dwServiceState, final String groupName) {
        ...
        final boolean result = Advapi32.INSTANCE.EnumServicesStatusEx(this._handle, 0, dwServiceType, dwServiceState, (Pointer)buffer, (int)buffer.size(), pcbBytesNeeded, lpServicesReturned, lpResumeHandle, groupName);
        if (!result) {
            throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
        }
        ...
        final Winsvc.ENUM_SERVICE_STATUS_PROCESS status = Structure.newInstance(Winsvc.ENUM_SERVICE_STATUS_PROCESS.class, (Pointer)buffer);
        status.read();
