...
public abstract class Advapi32Util
{
    ...
    public static Account getAccountByName(final String systemName, final String accountName) {
        ...
        final Memory sidMemory = new Memory((long)pSid.getValue());
        final WinNT.PSID result = new WinNT.PSID((Pointer)sidMemory);
    ...
    public static int getAceSize(final int sidLength) {
        return Native.getNativeSize((Class)WinNT.ACCESS_ALLOWED_ACE.class, (Object)null) + sidLength - 4;
    }
    ...
    public static Account[] getTokenGroups(final WinNT.HANDLE hToken) {
        ...
        if (!Advapi32.INSTANCE.GetTokenInformation(hToken, 2, (Structure)groups, tokenInformationLength.getValue(), tokenInformationLength)) {
            throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
        }
    ...
    public static Account getTokenAccount(final WinNT.HANDLE hToken) {
        ...
        if (!Advapi32.INSTANCE.GetTokenInformation(hToken, 1, (Structure)user, tokenInformationLength.getValue(), tokenInformationLength)) {
            throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
        }
            ...
            if (!WinBase.INVALID_HANDLE_VALUE.equals(hToken)) {
                try {
                    ...
                    if (err == null) {
                        ...
                        err.addSuppressedReflected((Throwable)e2);
                    }
                }
            }
            if (err != null) {
    ...
    public static String registryGetStringValue(final WinReg.HKEY hKey, final String value) {
        ...
        final Memory mem = new Memory((long)(lpcbData.getValue() + Native.WCHAR_SIZE));
        mem.clear();
    ...
    public static String[] registryGetStringArray(final WinReg.HKEY hKey, final String value) {
        ...
        final Memory data = new Memory((long)(lpcbData.getValue() + 2 * Native.WCHAR_SIZE));
        data.clear();
    ...
    public static Object registryGetValue(final WinReg.HKEY hkKey, final String subKey, final String lpValueName) {
        ...
        final Memory byteData = new Memory((long)(lpcbData.getValue() + Native.WCHAR_SIZE));
        byteData.clear();
    ...
    public static boolean registryCreateKey(final WinReg.HKEY hKey, final String keyName, final int samDesiredExtra) {
        ...
        int rc = Advapi32.INSTANCE.RegCreateKeyEx(hKey, keyName, 0, (String)null, 0, 0x20019 | samDesiredExtra, (WinBase.SECURITY_ATTRIBUTES)null, phkResult, lpdwDisposition);
        if (rc != 0) {
            throw new Win32Exception(rc);
        }
    ...
    public static void registrySetStringValue(final WinReg.HKEY hKey, final String name, String value) {
        ...
        final int rc = Advapi32.INSTANCE.RegSetValueEx(hKey, name, 0, 1, (Pointer)data, (int)data.size());
        if (rc != 0) {
            throw new Win32Exception(rc);
        }
    }
    ...
    public static void registrySetExpandableStringValue(final WinReg.HKEY hKey, final String name, final String value) {
        ...
        final int rc = Advapi32.INSTANCE.RegSetValueEx(hKey, name, 0, 2, (Pointer)data, (int)data.size());
        if (rc != 0) {
            throw new Win32Exception(rc);
        }
    }
    ...
    public static void registrySetStringArray(final WinReg.HKEY hKey, final String name, final String[] arr) {
        ...
        size += charwidth;
        int offset = 0;
        final Memory data = new Memory((long)size);
        data.clear();
        for (final String s2 : arr) {
            if (W32APITypeMapper.DEFAULT == W32APITypeMapper.UNICODE) {
                data.setWideString((long)offset, s2);
            }
            else {
                data.setString((long)offset, s2);
            }
            offset += s2.length() * charwidth;
            offset += charwidth;
        }
        final int rc = Advapi32.INSTANCE.RegSetValueEx(hKey, name, 0, 7, (Pointer)data, size);
        if (rc != 0) {
            throw new Win32Exception(rc);
        }
    }
    ...
    public static TreeMap<String, Object> registryGetValues(final WinReg.HKEY hKey) {
        ...
        final Memory byteData = new Memory((long)(lpcMaxValueLen.getValue() + 2 * Native.WCHAR_SIZE));
    ...
    public static InfoKey registryQueryInfoKey(final WinReg.HKEY hKey, final int lpcbSecurityDescriptor) {
        ...
        final int rc = Advapi32.INSTANCE.RegQueryInfoKey(hKey, infoKey.lpClass, infoKey.lpcClass, (IntByReference)null, infoKey.lpcSubKeys, infoKey.lpcMaxSubKeyLen, infoKey.lpcMaxClassLen, infoKey.lpcValues, infoKey.lpcMaxValueNameLen, infoKey.lpcMaxValueLen, infoKey.lpcbSecurityDescriptor, infoKey.lpftLastWriteTime);
        if (rc != 0) {
            throw new Win32Exception(rc);
        }
        return infoKey;
    ...
    public static EnumKey registryRegEnumKey(final WinReg.HKEY hKey, final int dwIndex) {
        ...
        final int rc = Advapi32.INSTANCE.RegEnumKeyEx(hKey, enumKey.dwIndex, enumKey.lpName, enumKey.lpcName, (IntByReference)null, enumKey.lpClass, enumKey.lpcbClass, enumKey.lpftLastWriteTime);
        if (rc != 0) {
            throw new Win32Exception(rc);
        }
        return enumKey;
    ...
    public static WinNT.ACE_HEADER[] getFileSecurity(final String fileName, final boolean compact) {
        ...
        do {
            ...
            final boolean succeded = Advapi32.INSTANCE.GetFileSecurity(fileName, infoType, (Pointer)memory, nLength, lpnSize);
            if (!succeded) {
        ...
        final WinNT.SECURITY_DESCRIPTOR_RELATIVE sdr = new WinNT.SECURITY_DESCRIPTOR_RELATIVE((Pointer)memory);
    ...
    private static Memory getSecurityDescriptorForFile(final String absoluteFilePath) {
        ...
        final Memory securityDescriptorMemoryPointer = new Memory((long)nLength);
    ...
    public static Memory getSecurityDescriptorForObject(final String absoluteObjectPath, final int objectType, final boolean getSACL) {
        ...
        final int lastError = Advapi32.INSTANCE.GetNamedSecurityInfo(absoluteObjectPath, objectType, infoType, (PointerByReference)null, (PointerByReference)null, (PointerByReference)null, (PointerByReference)null, ppSecurityDescriptor);
        if (lastError != 0) {
            throw new Win32Exception(lastError);
        }
        final int nLength = Advapi32.INSTANCE.GetSecurityDescriptorLength(ppSecurityDescriptor.getValue());
        final Memory memory = new Memory((long)nLength);
    ...
    public static boolean accessCheck(final File file, final AccessCheckPermission permissionToCheck) {
        ...
        try {
            ...
            final WinDef.DWORDByReference privilegeLength = new WinDef.DWORDByReference(new WinDef.DWORD(privileges.size()));
    ...
    public static WinNT.SECURITY_DESCRIPTOR_RELATIVE getFileSecurityDescriptor(final File file, final boolean getSACL) {
        ...
        final WinNT.SECURITY_DESCRIPTOR_RELATIVE sdr = new WinNT.SECURITY_DESCRIPTOR_RELATIVE((Pointer)securityDesc);
        return sdr;
    }
    ...
    public static boolean isCurrentProcessElevated() {
        ...
        if (Advapi32.INSTANCE.OpenProcessToken(Kernel32.INSTANCE.GetCurrentProcess(), 8, hToken)) {
            try {
                final WinNT.TOKEN_ELEVATION elevation = new WinNT.TOKEN_ELEVATION();
                if (Advapi32.INSTANCE.GetTokenInformation(hToken.getValue(), 20, (Structure)elevation, elevation.size(), returnLength)) {
        ...
        public Privilege enable() throws Win32Exception {
            ...
            try {
                ...
                if (this.currentlyImpersonating) {
                    ...
                    Advapi32.INSTANCE.AdjustTokenPrivileges(phThreadToken.getValue(), false, tp2, 0, (WinNT.TOKEN_PRIVILEGES)null, (IntByReference)null);
                    this.privilegesEnabled = false;
                }
                throw ex;
            }
