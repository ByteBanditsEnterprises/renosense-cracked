...
public class TypeLibUtil
{
    ...
    public TypeLibUtil(final String clsidStr, final int wVerMajor, final int wVerMinor) {
        ...
        COMUtils.checkRC(hr);
        this.typelib = (ITypeLib)new TypeLib(pTypeLib.getValue());
    ...
    public OaIdl.TYPEKIND getTypeInfoType(final int index) {
        ...
        final WinNT.HRESULT hr = this.typelib.GetTypeInfoType(new WinDef.UINT(index), typekind);
        COMUtils.checkRC(hr);
        return typekind;
    }
    ...
    public ITypeInfo getTypeInfo(final int index) {
        ...
        final WinNT.HRESULT hr = this.typelib.GetTypeInfo(new WinDef.UINT(index), ppTInfo);
        COMUtils.checkRC(hr);
    ...
    public IsName IsName(final String nameBuf, final int hashVal) {
        ...
        final WinDef.ULONG lHashVal = new WinDef.ULONG(hashVal);
    ...
    public static class FindName
    {
        ...
        public ITypeInfo[] getTInfo() {
            ...
            for (int i = 0; i < this.pcFound; ++i) {
                values[i] = (ITypeInfo)new TypeInfo(this.pTInfo[i]);
            }
            return values;
        }
