...
public class GDI32Util
{
    ...
    public static BufferedImage getScreenshot(final WinDef.HWND target) {
        ...
        try {
            ...
            hOriginal = GDI32.INSTANCE.SelectObject(hdcTargetMem, (WinNT.HANDLE)hBitmap);
            if (hOriginal == null) {
                throw new Win32Exception(Native.getLastError());
            }
            ...
            final int resultOfDrawing = GDI32.INSTANCE.GetDIBits(hdcTarget, hBitmap, 0, windowHeight, (Pointer)buffer, bmi, 0);
            ...
            if (hOriginal != null) {
                ...
                if (result == null || WinGDI.HGDI_ERROR.equals(result)) {
                    ...
                    if (we != null) {
                        ex.addSuppressedReflected((Throwable)we);
                    }
                    we = ex;
                }
            }
            if (hBitmap != null && !GDI32.INSTANCE.DeleteObject((WinNT.HANDLE)hBitmap)) {
                final Win32Exception ex2 = new Win32Exception(Native.getLastError());
                if (we != null) {
                    ex2.addSuppressedReflected((Throwable)we);
                }
                we = ex2;
            }
