...
public enum SQLiteErrorCode
{
    ...
    SQLITE_AUTH_USER(279, "An operation was attempted on a database for which the logged in user lacks sufficient authorization"), 
    ...
    SQLITE_BUSY_TIMEOUT(773, "A blocking Posix advisory file lock request in the VFS layer failed due to a timeout"), 
    ...
    SQLITE_CANTOPEN_SYMLINK(1550, "The file is a symbolic link but SQLITE_OPEN_NOFOLLOW flag is used"), 
    ...
    SQLITE_CONSTRAINT_DATATYPE(3091, "An insert or update attempted to store a value inconsistent with the column's declared type in a table defined as STRICT"), 
    ...
    SQLITE_CONSTRAINT_PINNED(2835, "An UPDATE trigger attempted to delete the row that was being updated in the middle of the update"), 
    ...
    SQLITE_ERROR_MISSING_COLLSEQ(257, "An SQL statement could not be prepared because a collating sequence named in that SQL statement could not be located"), 
    ...
    SQLITE_IOERR_BEGIN_ATOMIC(7434, "the underlying operating system reported and error on the SQLITE_FCNTL_BEGIN_ATOMIC_WRITE file-control"), 
    ...
    SQLITE_IOERR_COMMIT_ATOMIC(7690, "the underlying operating system reported and error on the SQLITE_FCNTL_COMMIT_ATOMIC_WRITE file-control"), 
    SQLITE_IOERR_CONVPATH(6666, "cygwin_conv_path() system call failed"), 
    SQLITE_IOERR_CORRUPTFS(8458, "I/O error in the VFS layer, a seek or read failure was due to the request not falling within the file's boundary rather than an ordinary device failure"), 
    SQLITE_IOERR_DATA(8202, "I/O error in the VFS shim, the checksum on a page of the database file is incorrect"), 
    ...
    SQLITE_IOERR_ROLLBACK_ATOMIC(7946, "the underlying operating system reported and error on the SQLITE_FCNTL_ROLLBACK_ATOMIC_WRITE file-control"), 
    ...
    SQLITE_OK_LOAD_PERMANENTLY(256, "the extension remains loaded into the process address space after the database connection closes"), 
    SQLITE_READONLY_CANTINIT(1288, "the current process does not have write permission on the shared memory region"), 
    ...
    SQLITE_READONLY_DIRECTORY(1544, "Process does not have permission to create a journal file in the same directory as the database and the creation of a journal file is a prerequisite for writing"), 
